...
func CreateAccessToken(priv *rsa.PrivateKey, accessTTL time.Duration, userID, role string) (string, error) {
	now := time.Now()
...
func CreateRefreshToken(priv *rsa.PrivateKey, refreshTTL time.Duration, userID string) (string, error) {
	now := time.Now()
